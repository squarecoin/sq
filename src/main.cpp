...
uint256 hashGenesisBlock("0xf864b1283b084ca516f3ee6b8b8adcee89a462d3a92aa7184108750d4eafa68a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Square: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Square Signed Message:\n";
...
{
    // Square: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 5000000 * 70  / 100 / 150;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Square: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Square: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Square: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf864b1283b084ca516f3ee6b8b8adcee89a462d3a92aa7184108750d4eafa68a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1404110726;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084695147;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SquareMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SquareMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Square with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
